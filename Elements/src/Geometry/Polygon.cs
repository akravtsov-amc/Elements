...
namespace Elements.Geometry
{
    ...
    public partial class Polygon : Polyline
    {
        ...
        private List<Vector3> approximateArc(Vector3 center, Vector3 start, Vector3 end, double stepAngle, Vector3 zAxis)
        {
            start -= center;
            double angleDiff = start.PlaneAngleTo(end - center, zAxis);
            int k = (int)Math.Ceiling(angleDiff / stepAngle);
            stepAngle = angleDiff / k;
            var points = new List<Vector3>();
            double q = Math.Cos(stepAngle * Math.PI / 360);
            for (int i = 0; i < k; ++i)
            {
                points.Add(center + new Transform(new Vector3(0, 0, 0), zAxis, stepAngle * i + stepAngle / 2).OfPoint(start) / q);
            }
            return points;
        }

        private double stepAngleForApproximation(double radius, double delta)
        {
            return 360 * Math.Acos(radius / (radius + delta)) / Math.PI;
        }

        public Polygon OffsetPolygon(double offset, double? delta = null)
        {
            if (offset == 0)
            {
            ...
            if (IsClockWise())
            {
                return Reversed().OffsetPolygon(offset, delta).Reversed();
            }
            ...
            return OffsetPolygon(offset, Normal(), stepAngleForApproximation(offset, delta ?? offset * 0.035));
        }

        private Polygon OffsetPolygon(double offset, Vector3 normal, double stepAngle)
        {
            ...
            if (n == 3)
            {
                ...
                offsetVector = (Vertices[1] - Vertices[0]).Cross(normal).Unitized() * offset;
                var p1 = Vertices[0] + offsetVector;
                var p2 = Vertices[1] + offsetVector;
                offsetVector = (Vertices[2] - Vertices[1]).Cross(normal).Unitized() * offset;
                var p3 = Vertices[1] + offsetVector;
                var p4 = Vertices[2] + offsetVector;
                offsetVector = (Vertices[0] - Vertices[2]).Cross(normal).Unitized() * offset;
                var p5 = Vertices[2] + offsetVector;
                var p6 = Vertices[0] + offsetVector;

                var points = new List<Vector3>();
                points.Add(p1);
                points.Add(p2);
                points.AddRange(approximateArc(Vertices[1], p2, p3, stepAngle, normal));
                points.Add(p3);
                points.Add(p4);
                points.AddRange(approximateArc(Vertices[2], p4, p5, stepAngle, normal));
                points.Add(p5);
                points.Add(p6);
                points.AddRange(approximateArc(Vertices[0], p6, p1, stepAngle, normal));
            ...
            do
            {
                ...
                var diagonal = new Line(Vertices[i1], Vertices[i3]);
                bool goodDiagonal = Math.Sign(normal.Dot((Vertices[i2] - Vertices[i1]).Cross(Vertices[i3] - Vertices[i1]))) == 1 && 
                                    Edges().Count(e => diagonal.Intersects(new Line(e.from, e.to), out _, includeEnds: true)) <= 4;
                if (goodDiagonal)
                {
                    var vertices2 = new List<Vector3>();
                    vertices2.AddRange(Vertices.Take(i2));
                    vertices2.AddRange(Vertices.Skip(i2 + 1));
                    return (new Polygon(vertices2)).OffsetPolygon(offset, normal, stepAngle).Union(new List<Polygon> { (new Polygon(Vertices[i1], Vertices[i2], Vertices[i3])).OffsetPolygon(offset, normal, stepAngle) });
                }
